...
function send_notification_email($ipn, $verified) {
    ...
    switch ($transaction) {
    case "web_accept":
        web_accept_data($output, $log, $ipn);
        break;
    case "recurring_payment":
        recurring_payment_data($output, $log, $ipn);
        break;
    case "recurring_payment_profile_created":
        recurring_payment_profile_created_data($output, $log, $ipn);
        break;
    default:
        unknown_ipn_data($output, $log, $ipn);
    }
 ...
 * 
*/
function web_accept_data(&$output, &$log, $ipn) {
    $output[] = $log['txn_desc'] = "Paypal transaction for regular (non-recurring) donation";
    $output[] = $log['donor_name'] = "Donor PayPal Name = " . $ipn["first_name"] . " " . $ipn["last_name"];
    $output[] = $log['donor_email'] = "Donor PayPal Email Address = " . $ipn["payer_email"];
    $output[] = $log['total'] = "Total Donation Amount = " . $ipn["mc_gross"];
    $output[] = $log['fee'] = "PayPal Payment Fee = " . $ipn["mc_fee"];

    if ($ipn["transaction_subject]"] == "") {
        $ipn["transaction_subject]"] = "Field left blank";
    }
    $output[] = $log['instructions'] = "Giving Instructions = " . $ipn["transaction_subject"];
}
 ...
 * 
*/
function recurring_payment_data(&$output, &$log, $ipn) {
    $output[] = $log['txn_desc'] = "Paypal transaction for recurring payment";
    $output[] = $log['donor_name'] = "Donor PayPal Name = " . $ipn["first_name"] . " " . $ipn["last_name"];
    $output[] = $log['donor_email'] = "Donor PayPal Email Address = " . $ipn["payer_email"];
    $output[] = $log['total'] = "Total Donation Amount = " . $ipn["payment_gross"];
    $output[] = $log['fee'] = "PayPal Payment Fee = " . $ipn["payment_fee"];

    if ($ipn["transaction_subject"] == "") {
        $ipn["transaction_subject"] = "Field left blank";
    }
    $output[] = "Giving Instructions = " . $ipn["transaction_subject"];
    $log['instructions'] = $ipn["transaction_subject"];
}
 ...
 *                                              
*/
function recurring_payment_profile_created_data(&$output, &$log, $ipn) {
    $output[] = $log['txn_desc'] = "Paypal transaction for the creation of a recurring payment";
    $output[] = $log['donor_name'] = "Donor PayPal Name = " . $ipn["first_name"] . " " . $ipn["last_name"];
    $output[] = $log['donor_email'] = "Donor PayPal Email Address = " . $ipn["payer_email"];
    $output[] = $log['total_per_cyle'] = "Donation Amount per cycle = " . $ipn["amount_per_cycle"];

    if ($ipn["product_name"] == "") {
        $ipn["product_name"] = "Field left blank";
    }
    $output[] = $log['instructions'] = "Giving Instructions = " . $ipn["product_name"];
}
 ...
 *                                              
*/
function unknown_ipn_data($output, &$log, $ipn) {
    $output[] = $log['unknown_ipn'] = "This is a Paypal IPN message we did not plan for. Please let support have a copy of this message";
